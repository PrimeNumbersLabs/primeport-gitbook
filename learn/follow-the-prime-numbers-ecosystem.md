# Community & Links

### Ecosystem

|                   | URL                                          |                                |
| ----------------- | -------------------------------------------- | ------------------------------ |
| **Prime Numbers** | [primenumbers.xyz](https://primenumbers.xyz) | Main ecosystem hub             |
| **PrimePort**     | [primeport.xyz](https://primeport.xyz)       | NFT marketplace (XDC + Base)   |
| **PrimeFi**       | [app.primefi.xyz](https://app.primefi.xyz)   | Lending and borrowing protocol |
| **Prime Staking** | [primestaking.xyz](https://primestaking.xyz) | PRFI ONFT staking platform     |

***

### Documentation

|                          |                                                                                       |
| ------------------------ | ------------------------------------------------------------------------------------- |
| **PrimePort Docs**       | [docs.primeport.xyz](https://docs.primeport.xyz)                                      |
| **Prime Staking Docs**   | [docs.primestaking.xyz](https://docs.primestaking.xyz/products/prfi-staking-nfts)     |
| **Whitepaper**           | [Prime Numbers Whitepaper](https://primenumberswhitepaper.gitbook.io/prime-numbers/)  |
| **PRFI Contract Source** | [GitHub — prfi-nft-contracts](https://github.com/PrimeNumbersLabs/prfi-nft-contracts) |

***

### PRFI ONFTs on Secondary

|               |                                                                                                                      |
| ------------- | -------------------------------------------------------------------------------------------------------------------- |
| **PrimePort** | [primeport.xyz/collection/base/primenumbers-prfi-onft](https://primeport.xyz/collection/base/primenumbers-prfi-onft) |
| **OpenSea**   | [opensea.io/collection/primenumbers-prfi-onft](https://opensea.io/collection/primenumbers-prfi-onft)                 |

***

### Social

| Platform          |                                                            |
| ----------------- | ---------------------------------------------------------- |
| X — Prime Numbers | [@PrimeNumbersFi](https://x.com/PrimeNumbersFi)            |
| X — PrimeFi       | [@PrimeFi](https://x.com/primefixyz)                       |
| Medium            | [@PrimeNumbersFi](https://medium.com/@PrimeNumbersFi)      |
| YouTube           | [@PrimeNumbersLabs](https://youtube.com/@PrimeNumbersLabs) |

***

### Community

|                   |                                                              |
| ----------------- | ------------------------------------------------------------ |
| **Telegram Chat** | [t.me/PrimeNumbersFi](https://t.me/PrimeNumbersFi)           |
| **Telegram News** | [t.me/PrimeNumbersChannel](https://t.me/PrimeNumbersChannel) |
